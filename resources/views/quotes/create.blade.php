...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-5xl mx-auto">
        <!-- En-tête -->
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Nouveau Devis</h1>
            <p class="text-gray-600 mt-1">Créez un devis estimatif pour votre client</p>
        </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" 
                                       name="date_devis" 
                                       value="{{ old('date_devis', date('Y-m-d')) }}"
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" 
                                       required>
                                @error('date_devis')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <textarea name="objet" 
                                          rows="3"
                                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
                                          placeholder="Ex: Fabrication et pose de 14 fenêtres en aluminium...">{{ old('objet') }}</textarea>
                                @error('objet')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
...
<script>
    ...
    function addItem() {
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div class="md:col-span-2">
                        ...
                        <select name="items[${itemIndex}][product_id]" 
                            ...
                            ${products.map(p => `<option value="${p.id}" data-name="${p.nom}" data-sku="${p.sku}" data-price="${p.prix_vente}">${p.nom} (${p.sku})</option>`).join('')}
                        </select>
                    </div>
        ...
        if (newSelect && window.SearchableSelect) {
            new SearchableSelect(newSelect, {
                ...
                onChange: (option) => {
                    // Déclencher updateDesignation après sélection
    ...
    // Ajouter un premier item au chargement
