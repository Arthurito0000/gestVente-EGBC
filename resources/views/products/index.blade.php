      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          ...
          <tr class="hover:bg-gray-50">
            <td class="px-4 py-2 text-sm font-mono text-gray-700">{{ $product->sku }}</td>
            <td class="px-4 py-2 text-sm text-gray-900">{{ $product->nom }}</td>
            <td class="px-4 py-2 text-sm text-right text-gray-700">{{ number_format($product->prix_achat, 2, ',', ' ') }} Fcfa</td>
            ...
            <td class="px-4 py-2 text-sm text-right">
              <div class="flex items-center justify-end gap-1">
                <!-- Voir -->
                <a href="{{ route('products.show', $product) }}" class="group relative p-2.5 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all duration-200 hover:shadow-sm" title="Voir le produit">
                ...
                <!-- Modifier -->
                <a href="{{ route('products.edit', $product) }}" class="group relative p-2.5 text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all duration-200 hover:shadow-sm" title="Modifier le produit">
                  <svg class="w-4 h-4 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <form action="{{ route('products.destroy', $product) }}" method="POST" class="inline delete-form">
                  @csrf
                  @method('DELETE')
                  <button type="button" class="delete-btn group relative p-2.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all duration-200 hover:shadow-sm" title="Supprimer le produit" data-product-name="{{ $product->nom }}">
                    <svg class="w-4 h-4 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
